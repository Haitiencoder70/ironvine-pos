...
WORKDIR /app/backend
CMD ["sh", "-c", "export NODE_ENV=production && export DIRECT_URL=$(node -e \"console.log((process.env.DATABASE_URL || '').replace('-pooler', '').replace('&pgbouncer=true', '').replace('?pgbouncer=true', '?'))\") && npx prisma migrate deploy && npm run start"]
