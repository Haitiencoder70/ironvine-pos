  ...
  return (
    <div className="p-4 sm:p-6 max-w-xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-100">{isNew ? 'Invite User' : 'Edit User'}</h1>
      ...
      <form onSubmit={handleSubmit} className="card-cinema rounded-2xl p-6 space-y-5">
        ...
        <div>
          ...
          <input
            ...
            disabled={!isNew}
            className={clsx(
              'w-full min-h-[44px] rounded-xl border px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/60',
              errors.email ? 'border-red-500/60 bg-red-500/10 text-slate-100' : 'border-white/10 bg-white/[0.06] text-slate-100',
              !isNew && 'bg-white/[0.03] cursor-not-allowed text-slate-500',
            )}
          />
          {errors.email && <p className="mt-1 text-xs text-red-400">{errors.email}</p>}
          {!isNew && <p className="mt-1 text-xs text-slate-500">Email cannot be changed here — managed via Clerk account settings</p>}
        </div>
        ...
        <div>
            ...
            {ROLES.map((r) => (
              <label
                ...
                className={clsx(
                  ...
                  form.role === r.value ? 'border-blue-500/60 bg-blue-500/10' : 'border-white/10 hover:bg-white/[0.04]',
          ...
          {isMe && <p className="mt-2 text-xs text-slate-500">You cannot change your own role</p>}
        </div>
        ...
        {!isNew && !isMe && (
          <div className="flex items-center justify-between p-4 bg-white/[0.04] rounded-xl">
            <div>
              <p className="text-sm font-medium text-slate-100">Active</p>
            ...
            <button
              ...
              className={clsx(
                ...
                form.isActive ? 'bg-blue-600' : 'bg-white/20',
              )}
              role="switch"
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link
            to="/settings"
            className="min-h-[44px] px-6 rounded-xl border border-white/10 text-slate-400 text-sm font-medium hover:bg-white/5 transition-colors flex items-center"
          >
            Cancel
          </Link>
        </div>
