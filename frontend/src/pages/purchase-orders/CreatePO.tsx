          ...
          <TouchCard padding="lg" className="border border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className={clsx(
                          ...
                          errors.vendorId ? 'border-red-400 focus:border-red-500' : 'border-white/10 hover:border-white/20'
                        )}
                      >
              ...
              <div className="md:col-span-2">
                <label className="text-sm font-medium text-slate-300 block mb-1.5">Vendor Notes (Optional)</label>
                <textarea
                  rows={2}
          ...
          <TouchCard padding="lg" className="border border-white/10">
            ...
            <div className="space-y-4">
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="p-4 bg-white/[0.04] border border-white/10 rounded-xl space-y-3">
                    {/* Header row */}
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {linkedOrderId && orderData?.data && (
                          <p className="text-xs text-blue-400 mt-0.5">For Order: {orderData.data.orderNumber}</p>
                        )}
                      </div>
                      <button
                        type="button"
                        onClick={() => remove(index)}
                        className="min-h-[36px] min-w-[36px] flex items-center justify-center rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-400 transition-colors flex-shrink-0"
          ...
          <div className="flex justify-end p-4">
            <div className="w-full md:w-64 glass-panel p-4 rounded-xl border border-white/10">
               <div className="flex justify-between items-center text-sm text-slate-400 mb-2">
                 <span>Subtotal</span>
                 <span className="font-mono text-slate-100">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(subtotal)}</span>
               </div>
